...
class Trainer():
    def __init__(self,final, num_workers):
        self.context_cat_features = []
        self.context_num_features = []
        
        
        self.alt_num_features = ['attr1','attr2']
        
        self.alt_cat_features = []
    ...
    def load_data(self,fname):
        ...
        train_size = 3*round(learnset.shape[0]/3 * .8)

        ## all sessions contain 3 alternatives
        trainset = learnset[:train_size]
        devset = learnset[train_size:]
    
        trainset = [df for _,df in trainset.groupby("indiv")]
        devset = [df for _,df in devset.groupby("indiv")]
...
if __name__ == "__main__":
    ...
    parser.add_argument ('--opt_dropout', nargs=2, type=float)
